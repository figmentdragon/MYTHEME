<?php
if(!class_exists('MYTHEME_Welcome')) :

	class MYTHEME_Welcome {
			...
			public function __construct( $plugins, $strings ) {
				...
				$this->tab_sections = array(
					'getting_started' => esc_html__('Getting Started', 'MYTHEME'),
					'actions_required' => esc_html__('Recommended Plugins', 'MYTHEME'),
					
					'demo_import' => esc_html__('Import Demo', 'MYTHEME'),
					'free_vs_pro' => esc_html__('Free Vs Pro', 'MYTHEME'),
					'changelog' => esc_html__('ChangeLog', 'MYTHEME'),
					'support' => esc_html__('Support', 'MYTHEME'),
				);
		...
		if( is_admin() && ('themes.php' == $pagenow) && (isset($_GET['activated'])) ) {
			...
			<div class="updated apwelcome-an notice notice-success is-dismissible">
				<h2><?php printf(esc_html__( 'Welcome!', 'MYTHEME' )) ?></h2>
				<p class="about-desc"><?php /* translators: %s : Theme Name */ printf(esc_html__( 'Thank you for installing %1$s. It is now installed and ready to use. Here are some useful links to get started: ', 'MYTHEME' ), $this->theme_name);  ?></p>
				<hr/>
					...
					<div class="apwelcome-column col-demos">
						<div>
							<h3>
								...
								<?php printf(esc_html__( 'Starter site demos!', 'MYTHEME' )) ?>
							</h3>
							<p><?php printf( wp_kses_post( 'You\'ve  got 1  great starter sites (Demos) to start with. By using one of these starter sites you\'ll get a great guidance on how to use the theme to its fullest and also save time to make your website. ', 'MYTHEME' ), $this->theme_name, esc_url(admin_url( 'themes.php?page=welcome-page#demo_import' ) ) ); ?></p>
						</div>
						<div>
							<div class="button-wrapper">
								<a class="button button-primary button-hero install-now" href="<?php echo esc_url(admin_url( 'themes.php?page=welcome-page#demo_import' )) ?>"><?php esc_html_e( 'Ready to use starter sites.', 'MYTHEME' ); ?></a>
								<?php printf( wp_kses_post( '<a class="options-page-btn notice-info-btn button-secondary" href="%2$s"> Or Start setting up your theme now (without demo)!</a>', 'MYTHEME' ), $this->theme_name, esc_url(admin_url( 'themes.php?page=welcome-page' ))  ); ?>
							</div>
						</div>
					</div>
					<div class="apwelcome-column col-doc">
						<div>
							<h3>
								...
								<?php printf(esc_html__( 'Documentation', 'MYTHEME' )) ?>
							</h3>
							<p><?php printf( wp_kses_post( 'How to use %1$s! Here we\'ve a full and detailed documentation that explains how to use %1$s in its best. ', 'MYTHEME' ), $this->theme_name  ); ?></p>

							<div class="apwelcome-doc-btn">
								<a href="<?php echo esc_url($this->strings['doc_link']);?>" class=" button" ><span><?php esc_html_e( 'Full Documentation', 'MYTHEME' ); ?></span></a>
								<a href="<?php echo esc_url('https://accesspressthemes.com/support/')?>" class=" ti-return-dashboard  button button-secondary  install-now" ><span><?php esc_html_e( 'Contact Support', 'MYTHEME' ); ?></span></a>
							</div>
						</div>
		...
		<div class="wrap about-wrap access-wrap">
						...
						<div class="welcome-section-wrapper is_loading">
							...
							<div class="notice-sidebar is_loading">
								<div class= "notice-sidebar-item">
									<h4><?php echo esc_html('Join in our social networks!','MYTHEME') ?></h4>
									<p><?php echo esc_html__('Get connected, share your opinions and more via our social community: ', 'MYTHEME'); ?></p>

									<p><a href="<?php echo esc_url('https://www.facebook.com/AccessPressThemes')?>"><?php echo esc_html('Join our Facebook Group','MYTHEME') ?></a><?php echo esc_html__(' -to receive updates, offers and more.', 'MYTHEME'); ?></p>

									<p><a href="<?php echo esc_url('https://www.youtube.com/channel/UCzD8I1moKISXDjAVr8dIf-w')?>"><?php echo esc_html('Subscribe our YouTube Channel','MYTHEME') ?></a><?php echo esc_html__(' -for tutorials, videos and more.', 'MYTHEME'); ?></p>

									<p><a href="<?php echo esc_url('https://twitter.com/apthemes')?>"><?php echo esc_html('Follow us on Twitter','MYTHEME') ?></a><?php echo esc_html__(' -to stay updated. ', 'MYTHEME'); ?></p>
								</div>
								<div class= "notice-sidebar-item">
									<h4><?php echo esc_html('Rate us!','MYTHEME') ?></h4>
									<p><?php /* translators: %s : Theme Name */ printf(esc_html__('Are you enjoying %1$s? We would love to hear your feedback!', 'MYTHEME'),$this->theme_name); ?></p>
									<a href="https://wordpress.org/support/theme/MYTHEME/reviews/#new-post"><?php echo esc_html('Write a review','MYTHEME') ?></a>
			...
			public function plugin_installer_callback(){
				...
				if ( ! current_user_can('install_plugins') ) {
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'MYTHEME' ) );
				}
				...
				if (! wp_verify_nonce( $nonce, 'plugin_installer_nonce' )) {
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'MYTHEME') );
				}
			...
			public function plugin_activation_callback() {
				...
				if(file_exists($plugin_file)) {
					...
					echo esc_html__('Plugin Does not Exists' , 'MYTHEME');
				}
